...
# Forked from Agent Substrate's hack/create-kind-cluster.sh, to add
# KIND_WORKER_NODES and print the registry address to stdout for callers to
# capture.
...
worker_nodes="${KIND_WORKER_NODES:-3}"
KIND_BIN="${KIND_BIN:-kind}"

# Runs $KIND_BIN with the cwd it needs when it's a checkout-relative wrapper
# (e.g. an Agent Substrate checkout's own hack/kind.sh, which resolves its
# own tool binary off the current directory, not its own script path)
# rather than a plain binary on PATH.
run_kind() {
...
if [[ $# -gt 0 ]]; then
  case "$1" in
    -h|--help)
      echo "Usage: $0"
      echo "Creates the kind cluster '${KIND_CLUSTER_NAME}' and a local registry container on port ${reg_port}, and prints the registry address to stdout."
      echo
      echo "Configured through the environment:"
      echo "  KIND_CLUSTER_NAME  Name of the cluster to create (default: kind)."
      echo "  KIND_WORKER_NODES  Worker nodes alongside the one control-plane (default: 3)."
...
if ! [[ "${worker_nodes}" =~ ^[0-9]+$ ]]; then
  echo "error: KIND_WORKER_NODES must be a non-negative integer (got '${worker_nodes}')" >&2
  exit 1
fi
...
echo "Setting up local docker registry '${reg_name}' on port ${reg_port}..." >&2
if [ "$(docker inspect -f '{{.State.Running}}' "${reg_name}" 2>/dev/null || true)" == "true" ]; then
  if ! docker port "${reg_name}" | grep -q "${reg_port}"; then
    echo "Registry exists but is not mapped to port ${reg_port}. Recreating..." >&2
...
echo "Probing for /dev/kvm in the Docker environment..." >&2
HAS_KVM=0
if docker run --rm --device /dev/kvm busybox true >/dev/null 2>&1; then
  HAS_KVM=1
  echo "/dev/kvm found: micro-VM (kata + cloud-hypervisor) support will be enabled." >&2
else
  echo "/dev/kvm not available: micro-VM support disabled (gVisor still works)." >&2
fi

echo "Creating kind configuration for cluster '${KIND_CLUSTER_NAME}' (ipFamily=${IP_FAMILY}, ${worker_nodes} worker node(s))..." >&2
kind_config="${ROOT}/bin/kind-config.yaml"
cat <<EOF > "${kind_config}"
kind: Cluster
apiVersion: kind.x-k8s.io/v1alpha4
nodes:
EOF
# extraMounts is per-node: every node needs its own copy of this stanza,
# control-plane and workers alike, so micro-VM (kata + cloud-hypervisor)
# worker pods can use KVM regardless of which node they land on.
emit_node() {
  echo "- role: $1" >> "${kind_config}"
  if [ "${HAS_KVM}" = "1" ]; then
    cat <<EOF >> "${kind_config}"
  extraMounts:
  - hostPath: /dev/kvm
...
echo "Deleting existing kind cluster '${KIND_CLUSTER_NAME}' if it exists..." >&2
run_kind delete cluster --name "${KIND_CLUSTER_NAME}" || true
...
if [[ "${IP_FAMILY}" != "ipv4" &&
  ...
  echo "The 'kind' Docker network exists without IPv6; recreating it..." >&2
...
echo "Creating kind cluster '${KIND_CLUSTER_NAME}'..." >&2
run_kind create cluster --name "${KIND_CLUSTER_NAME}" --config "${kind_config}" >&2
...
if [[ "${IP_FAMILY}" == "ipv6" ]]; then
  ...
  if [[ "${server}" == "https://[::1]:"* ]]; then
    echo "Repointing the kubeconfig for '${KUBECTL_CONTEXT}' at localhost..." >&2
...
echo "Enabling Proxy ARP/NDP on kind nodes..." >&2
for node in $(run_kind get nodes --name "${KIND_CLUSTER_NAME}"); do
  ...
  docker exec "${node}" sysctl net.ipv4.conf.all.proxy_arp=1 >&2
  docker exec "${node}" sysctl -e net.ipv6.conf.all.proxy_ndp=1 >&2
done
...
if [ "${HAS_KVM}" = "1" ]; then
  echo "Preparing kind nodes for micro-VM (kata + cloud-hypervisor) runtime..." >&2
  for node in $(run_kind get nodes --name "${KIND_CLUSTER_NAME}"); do
    docker exec "${node}" chmod 666 /dev/kvm >&2
  done
fi

# 3. Add the registry config to the nodes
echo "Adding registry config to kind nodes..." >&2
...
echo "Connecting local registry to cluster network..." >&2
if [ "$(docker inspect -f='{{json .NetworkSettings.Networks.kind}}' "${reg_name}")" = "null" ]; then
  docker network connect "kind" "${reg_name}"
fi

# 5. Document the local registry in kube-public ConfigMap
echo "Documenting local registry in cluster..." >&2
cat <<EOF | kubectl --context="${KUBECTL_CONTEXT}" apply -f - >&2
apiVersion: v1
kind: ConfigMap
metadata:
...
# The registry address is this script's sole stdout output, for callers to
# capture via $(...).
